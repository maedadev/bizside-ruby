...
class Bizside::ConfigTest < ActiveSupport::TestCase
  ...
  def test_存在しない属性のget
    assert Bizside::Config.new().any_attr.blank?
  end
  ...
  def test_存在する属性のset
    config = Bizside::Config.new({ 'any_attr' => 123 })
    config.any_attr(456)
    assert_equal 123, config.any_attr # 変わらない
  end

  def test_存在する名前空間_存在しない属性_のget
    assert Bizside::Config.new({ 'any_namespace' => {} }).any_namespace.any_attr.blank?
  end

  def test_存在する名前空間_存在しない属性_の代入
  ...
  def test_存在する名前空間_存在しない属性_のset
    config = Bizside::Config.new({ 'any_namespace' => {} })
    config.any_namespace.any_attr('some value')
    assert_equal 'some value', config.any_namespace.any_attr
  end

  def test_存在する名前空間_存在する属性_のget
    assert_equal 123, Bizside::Config.new({ 'any_namespace' => { 'any_attr' => 123 } }).any_namespace.any_attr
  end

  def test_存在する名前空間_存在する属性_の代入
  ...
  def test_存在する名前空間_存在する属性_のset
    config = Bizside::Config.new({ 'any_namespace' => { 'any_attr' => 123 } })
    config.any_namespace.any_attr(456)
    assert_equal 123, config.any_namespace.any_attr # 変更なし
  end
end
