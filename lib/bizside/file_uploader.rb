...
module Bizside
  ...
  class FileUploader < CarrierWave::Uploader::Base
    ...
    def download_file_from_fog(path)
      ...
      raise Exception.new('File download failed.') unless system("curl '#{file.url}' -o '#{tmp_path}'")
      tmp_path
    end
    ...
    def download_file_from_fog_with_cache(path)
      ...
      unless File.exist?(cache_path)
        ...
        raise Exception.new('Failed to move file.') unless system("mv '#{tmp_path}' '#{cache_path}'")
      end
      cache_path
    end
